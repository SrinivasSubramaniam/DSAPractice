...
public class P62_MinimumWindowSubString {
	 ...
	 * 
	 */
	public String minWindowAsciiOptimised(String str, String target) {
		 if (target.length()>str.length()||str.length()==0 || target.length()==0) return "";
	        ...
	        int left=0,right=0,start=0,end=0,min=str.length();
	        boolean flag=false;
	...
	public String minWindowMapOptimised(String str, String target) {
        if (target.length()>str.length()||str.length()==0 || target.length()==0) return "";
       ...
       for (int i=0;i<target.length();i++){
        ...
        int left=0,right=0,start=0,end=0,min=str.length(),count=tmap.size();
        boolean flag=false;
        while (right<str.length()){
            char c=str.charAt(right++);
            ...
            while (count==0){
                ...
                if (tmap.containsKey(cleft)){
                    tmap.put(cleft,tmap.getOrDefault(cleft,0)+1);
                    if (tmap.get(cleft)>0) count++;
                }
            }
